...
　　​![image](https://image.peterjxl.com/blog/image-20230501152543-5rch27o.png)​
...
　　我们不再自己实现IoC了，因此`BeanFactory`​可以删掉；beans.properties同理也删掉
...
　　​​![image](https://image.peterjxl.com/blog/image-20230501154410-glt9sr1.png)​​

　　‍

　　第一个AOP，我们学到AOP的时候采用到；而最后一个jcl，其实就是日志依赖Jakarta Commons Logging，我们之前其实讲过：[常见的日志框架-简单介绍](https://www.peterjxl.com/JAVAEE/log-popular-framework/#jcl-jakarta-commons-logging)

　　​![image](https://image.peterjxl.com/blog/image-20230501154548-bdttkdg.png)​

　　‍

　　我们可以在Maven视图中，分析这个依赖的关系：

　　​​​![image](https://image.peterjxl.com/blog/image-20230501155229-fntzsa1.png)​​​

　　‍

　　可以看到如下结构：

　　​![image](https://image.peterjxl.com/blog/image-20230501155218-vl2hvsv.png)​

　　‍

　　这个图是不是在我们介绍Spring时，用到的图很类似？

　　​![image](https://image.peterjxl.com/blog/image-20230501155413-1il89kp.png)​
...
　　我们新建一个配置文件，bean.xml，其实也可以叫其他名字，只要不包含中文；然后我们导入约束，可以去docs\spring-framework-reference\index.html这里找：我们点击core

　　​![image](https://image.peterjxl.com/blog/image-20230501155927-92vcq8k.png)​

　　‍

　　然后我们搜索xmlns：可以看到有相关的约束，这里我们用第一个就行

　　​![image](https://image.peterjxl.com/blog/image-20230501160113-pjkft11.png)​
...
　　接下来我们就是获取到容器，然后根据id获取里面的对象了。在Spring中，我们并不会直接使用BeanFactory，而是ApplicationContext，这是一个接口，实现类有`ClassPathXmlApplicationContext`​，`FileSystemXmlApplicationContext`​和`AnnotationConfigApplicationContext`​，前面两个是基于配置文件的，后面是基于注解的，这里我们用第一个

　　​![image](https://image.peterjxl.com/blog/image-20230501162359-3ccj04n.png)​
...
　　之前我们都是创建我们自己项目中的bean；实际开发中，经常会用到第三方依赖，此时要放入Spring容器进行管理，还是有点难度的，此时就涉及到其他创建Bean对象的方式
...
　　而实际开发中，我们经常会遇到没有默认构造函数的类，例如一些第三方的工具类；如果我们修改其源码，加上构造函数，也太麻烦了！此时我们可以调用其提供的构造方法，并使用set属性作为构造方法的函数。
