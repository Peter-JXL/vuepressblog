...
2. 通过读取配置文件中配置的内容，反射创建对象。配置文件可以是xml也可以是properties
...
1. 我们可以用一个map来保存，我们称之为容器；
2. 在初始化的时候，将所有bean对象初始化并存到bean中；
