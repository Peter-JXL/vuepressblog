...
　　有时候，我们可能需要定义多个常量，并且这些常量是互相关联的：例如定义周一到周日这7个常量，可以用7个不同的`int`​表示：
...
　　当然，也可以定义为String类型。但无论是`int`​常量还是`String`​常量，使用这些常量来表示一组枚举值的时候，有一个严重的问题就是，编译器无法检查每个值的合理性，例如：
...
* 注意到`Weekday类`​定义的常量范围是`0`​~`6`​，并不包含`7`​，编译器无法检查不在枚举中的`int`​值；
...
final class WeekDay extends java.lang.Enum<WeekDay> {
