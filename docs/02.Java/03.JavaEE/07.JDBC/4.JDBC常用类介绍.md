...
　　说起一加载就执行的代码，读者们想到了什么？[static](https://www.peterjxl.com/JavaSE/static/#%E5%89%8D%E8%A8%80)代码块。通过查看MySQL驱动类的源码发现：在`com.mysql.cj.jdbc.Driver`​类中存在静态代码块，会执行register方法。
...
　　​![image](https://image.peterjxl.com/blog/image-20230322071504-0v8bz0y.png)​
...
　　​![image](https://image.peterjxl.com/blog/image-20230322072646-qeyoxdo.png)​
...
* `boolean execute(String sql)`​：可以执行任意的sql  （了解即可，较少使用）
* `int executeUpdate(String sql)`​：执行DML语句（例如insert、update、delete）、DDL语句(例如create，alter、drop)。返回值：影响的行数，可以通过这个影响的行数判断DML语句是否执行成功 。返回值>0的则执行成功，反之，则失败。执行DDL语句则返回0。
...
　　​![image](https://image.peterjxl.com/blog/image-20230322195311-5c1jfad.png)​
